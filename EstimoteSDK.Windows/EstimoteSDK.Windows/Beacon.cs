...
namespace EstimoteSDK.Windows
{
    ...
    public class Beacon : INotifyPropertyChanged
    {
        ...
        private readonly Guid _telemetryGuid = new Guid("0000FE9A-0000-1000-8000-00805F9B34FB");
        ...
        public enum BeaconTypeEnum
        {
            ...
            iBeacon,
            /// <summary>
            /// Beacon conforming to the Estimote Telemetry specification.
            /// Nearble is the protocole that the Telemetry is using.
            /// </summary>
            Telemetry
        }
        ...
        public void UpdateBeacon(BluetoothLEAdvertisementReceivedEventArgs btAdv)
        {
            ...
            if (btAdv.Advertisement.ServiceUuids.Any())
            {
                ...
                {
                    // Bluetooth devices services UUID
                    //Debug.WriteLine("Service UUID : " + serviceUuid.ToString() + " Data type : " + btAdv.Advertisement.DataSections[0].DataType.ToString("X")
                    //    + " Data : " + BitConverter.ToString(btAdv.Advertisement.DataSections[0].Data.ToArray()));
                    ...
                    if (BeaconType == BeaconTypeEnum.Unknown)
                    {
                        ...
                        {
                            BeaconType = BeaconTypeEnum.Eddystone;
                        }
                        else if (serviceUuid.Equals(_telemetryGuid))
                        {
                            BeaconType = BeaconTypeEnum.Telemetry;
                        }
                        else
                        {
                            BeaconType = BeaconTypeEnum.Unknown;
                        }
            ...
            if (btAdv.Advertisement.DataSections.Any())
            {
                ...
                else if (BeaconType == BeaconTypeEnum.Telemetry)
                {
                    // This beacon is according to the Estimote Telemetry specification - parse data
                    ParseTelemetryData(btAdv);
                }
            ...
            if (btAdv.Advertisement.ManufacturerData.Any())
            {
                foreach (var manufacturerData in btAdv.Advertisement.ManufacturerData)
                {
                    ...
                    if (manufacturerData.CompanyId == 0x4C && manufacturerData.Data.Length >= 23 && manufacturerDataArry[0] == 0x02)
                    {
        ...
        private void ParseTelemetryData(BluetoothLEAdvertisementReceivedEventArgs btAdv)
        {
            // Parse Telemetry data
            foreach (var dataSection in btAdv.Advertisement.DataSections)
            {
                //Debug.WriteLine("Telemetry Beacon data: " + dataSection.DataType + " = " +
                //                BitConverter.ToString(dataSection.Data.ToArray()));
                //+" (" + Encoding.UTF8.GetString(dataSection.Data.ToArray()) + ")\n");

                // Relvant data of Telemetry is in data section 0x22
                // Windows receives: 0x01 = 0x06 or 0x04
                //                   0x03 = 0x9A 0xFE
                //                   0x22 = 0x9A 0xFE [data]
                if (dataSection.DataType == 22)
                {
                    var beaconFrame = dataSection.Data.ToArray().CreateTelemetryBeaconFrame();
                    ...
                    for (var i = 0; i < BeaconFrames.Count; i++)
                    {
                        if (BeaconFrames[i].GetType() == beaconFrame.GetType())
                        {
                            ...
                            if (beaconFrame.GetType() == typeof(UnknownBeaconFrame))
                            {
                                ...
                                var existingTelemetryFrameType = BeaconFrames[i].Payload.IsTelemetryFrameType();
                                var newTelemetryFrameType = beaconFrame.Payload.IsTelemetryFrameType();
                                if (existingTelemetryFrameType != false &&
                                    existingTelemetryFrameType == newTelemetryFrameType)
                                {
                                    updateFrame = true;
                                }
                            }
        ...
        private void ParseEddystoneData(BluetoothLEAdvertisementReceivedEventArgs btAdv)
        {
            ...
            foreach (var dataSection in btAdv.Advertisement.DataSections)
            {
                //Debug.WriteLine("Eddystone Beacon data: " + dataSection.DataType + " = " +
                //                BitConverter.ToString(dataSection.Data.ToArray()));
                //+ " (" + Encoding.UTF8.GetString(dataSection.Data.ToArray()) + ")\n");
